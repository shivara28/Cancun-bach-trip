...
const PackingList = () => {
    const defaultEssentials = [
        ...
        'Undergarments & Socks', 'PJs',
        'Toothbrush & Toothpaste', 'Deodorant', 'Skincare & Makeup', 'Makeup Remover',
        'Hair Tools (Straightener/Curler)', 'Hair Ties & Bobby Pins', 'Brush/Comb',
        'Razor', 'Lotion',
        'Advil / Tums / Pepto', 'Band-aids', 'Vitamins / Meds',
        'Glasses', 'Contacts & Solution',
        'Phone Charger', 'Portable Charger', 'Headphones', 'Waterproof Phone Case'
    ...
    useEffect(() => {
        ...
        if (savedEssentials) {
            ...
            if (parsed.length > 0 && typeof parsed[0] === 'string') {
                setEssentials(parsed.map(item => ({ text: item, checked: false })));
            } else {
    ...
    const handleAddItem = () => {
        if (newItem.trim()) {
            setEssentials([...essentials, { text: newItem.trim(), checked: false }]);
            setNewItem('');
        }
    };
    ...
    const handleEditItem = (index, value) => {
        const newEssentials = [...essentials];
        newEssentials[index] = { ...newEssentials[index], text: value };
        setEssentials(newEssentials);
    };
    ...
    const defaultThemes = [
        ...
        },
        {
            ...
            items: [
                { label: 'Daytime', desc: 'Black Bikinis for the squad and blue bikini for Simmu' },
                { label: 'Dinner', desc: 'Cocktail Dress (No specific theme, just don\'t wear white and look hot).' },
                { label: 'Night Out', desc: 'Mermaid & Matrimony Theme! Blue/Green fits (ocean/kelp) and Simmu in Sequin white Dress' }
            ],
            color: 'bg-blue-50',
            ...
            <div className="max-w-3xl mx-auto px-4">
                ...
                {themes.some(t => t.video) && (
                    <div className="mb-12">
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            {themes.filter(t => t.video).map((theme, idx) => (
                                <div key={idx} className="bg-white p-4 rounded-2xl shadow-sm border border-teal-50">
                                    <p className="text-sm font-bold text-teal-900 mb-3">{theme.title}</p>
                                    <blockquote
